    ...
    public void paint(Graphics g) {
        ...
        while (it.hasNext()) {
            ...
            if (((MemoryPresenter) presenter).getMemProcessColor(start) != null) {
                ...
                if (prog_height != height)
                    Functions.getInstance().drawTexture(g2, FRAG_I, ADDR_WIDTH, memStart + prog_height, w - ADDR_WIDTH - BORDER, height - prog_height);
            } else {
                // Draw empty block
                Color bground = EMPTY;
            ...
            map.put(new Rectangle2D.Double(ADDR_WIDTH, memStart, w - ADDR_WIDTH - BORDER, height), Integer.valueOf(start));
        }
    }
