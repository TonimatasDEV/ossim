...
public class FormProcess extends FormTemplate {
    ...
    public void init(Vector<Object> values) {
        ...
        if (values.size() > 1)
            spmodel = new SpinnerNumberModel(Integer.parseInt(values.get(2).toString()), 1, MAX_PROGSIZE, 1);
        ...
        if (values.size() > 1)
            spmodelDuration = new SpinnerNumberModel(Integer.parseInt(values.get(3).toString()), -1, 100, 1);
