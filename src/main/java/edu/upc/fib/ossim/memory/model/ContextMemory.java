...
public class ContextMemory {
    ...
    private final List<ProcessMemUnit> processQueue;    // Processes arriving, creation ordered 
    private final List<MemPartition> memory;    // Memory structure, ordered by init address 
    private final List<ProcessMemUnit> swap;    // Processes swapped out

    private final List<ProcessMemUnit> pqBkup;        // Programs arriving backup to restore initial state  
    private final List<MemPartition> bqBkup;        // Memory structure backup to restore initial state
    ...
    public boolean setSelectedPartition(int start, boolean started) {
        ...
        if (selectedPartition.getAllocated() != null && selectedPartition.getAllocated().getParent().getPid() == 0)
            return false;
        if (started) {
            return selectedPartition.getAllocated() != null;
    ...
    public Iterator<Integer> iteratorProcesses() {
        ...
        while (it.hasNext()) queueInteger.add(Integer.valueOf(it.next().getPid()));
    ...
    public Iterator<Integer> iteratorPartitions() {
        ...
        while (it.hasNext()) queueInteger.add(Integer.valueOf(it.next().getStart()));
    ...
    public void addMemPartition(Vector<Object> data) throws SoSimException {
        ...
        while (it.hasNext() && !end) {
            ...
            if (aux.getStart() < b.getStart() && (aux.getStart() + aux.getSize()) > b.getStart())
                throw new SoSimException("me_05");
            if (b.getStart() < aux.getStart() && (b.getStart() + b.getSize()) > aux.getStart())
                throw new SoSimException("me_05");
        }
