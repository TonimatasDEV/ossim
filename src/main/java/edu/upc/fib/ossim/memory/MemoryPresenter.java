...
import edu.upc.fib.ossim.memory.model.*;
import edu.upc.fib.ossim.memory.view.*;
...
public class MemoryPresenter extends Presenter {
        ...
        switch (action) {
            case 60:
                ...
                if (context.getProcessCount() >= ContextMemory.MAX_PROCESSES)
                    throw new SoSimException("me_15", "(max. : " + ContextMemory.MAX_PROCESSES + ")");
            ...
            case 64:
                ...
                if (timecontrols.isRunning())
                    JOptionPane.showMessageDialog(panel, Translation.getInstance().getError("me_01"), "Error", JOptionPane.ERROR_MESSAGE);
                else {
    ...
    private boolean confirmChange(boolean change) {
        return change && ((!context.iteratorProcesses().hasNext() ||
            ...
            switch (child) {
                ...
                case 1:   // Memory blocks
                    for (int i = 0; i < data.size(); i++) {
                        ...
                        blockData.add(Integer.valueOf(block.get(0).get(1))); // start. Value at position 1
                        blockData.add(Integer.valueOf(block.get(1).get(1))); // size. Value at position 1
                ...
                case 2:  // Programs
                    for (int i = 0; i < data.size(); i++) {
                        ...
                        programData.add(Integer.valueOf(program.get(2).get(1))); // size. Value at position 1
                        programData.add(Integer.valueOf(program.get(3).get(1))); // duration. Value at position 1
                        ...
                        if (program.size() > 5) { // Components
                            ...
                            for (int j = 0; j < num; j++) {
                                ...
                                component.add(Integer.valueOf(program.get(k + j * 3).get(1)));    // bid
                                component.add(Integer.valueOf(program.get(k + j * 3 + 1).get(1)));    // size
                                component.add(Boolean.valueOf(program.get(k + j * 3 + 2).get(1)));    // load?
