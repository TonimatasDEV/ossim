...
public class HelpDialog extends EscapeDialog implements HyperlinkListener {
    ...
    private final JSplitPane splitPn;
    ...
    private void externalResource(URL url) {
        ...
        if (AppSession.getInstance().getApp().allowOpenSave()) {
            ...
            if (Desktop.isDesktopSupported()) { // Comprovar suport SO
                ...
                JOptionPane.showMessageDialog(parent, Translation.getInstance().getError("all_05"), "Error", JOptionPane.ERROR_MESSAGE);
        } else
            JOptionPane.showMessageDialog(parent, Translation.getInstance().getError("all_10"), "Error", JOptionPane.ERROR_MESSAGE);
    }
} 
