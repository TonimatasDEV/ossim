...
public class FileSystemStrategyFAT extends FileSystemStrategyAdapter {
    private final int[] fatTable;    // Fat Table. -1 indicatesd last block
    ...
    public Vector<Vector<Object>> getDetailInfoData(Block[] device, PhysicalFSObject o) {
        ...
        do {
            ...
            row.add(new ColorCell(Integer.toString(next), item.getColor()));
            if (fatTable[next] == -1)
                row.add(new ColorCell(Translation.getInstance().getLabel("fs_47"), item.getColor()));
            else row.add(new ColorCell(Integer.toString(fatTable[next]), item.getColor()));
            data.add(row);
    ...
    public Vector<Vector<Object>> getTableInfoData(Block[] device) {
        ...
        for (int i = getFirstDataBlock(); i < fatTable.length; i++) {
            ...
            else row.add(new ColorCell(Integer.toString(fatTable[i]), color));
