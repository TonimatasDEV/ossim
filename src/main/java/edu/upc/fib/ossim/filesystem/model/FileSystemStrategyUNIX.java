...
public class FileSystemStrategyUNIX extends FileSystemStrategyAdapter {
    ...
    private final Inode[] inodesTable;    // Inodes Table.
    ...
    private Vector<Vector<Object>> getInodeInfoData(Inode inode, Color color) {
        ...
        row.add(new ColorCell(Integer.toString(inode.getLinks()), color));
        data.add(row);
        ...
        for (int i = 0; i < Inode.DIRECT_BLOCKS; i++) {
            ...
            if (direct.size() > i) row.add(new ColorCell(Integer.toString(direct.get(i).getId()), color));
        ...
        if (inode.getIndirect_1() != null)
            row.add(new ColorCell(Integer.toString(inode.getIndirect_1().getId()), color));
        else row.add(new ColorCell(Translation.getInstance().getLabel("fs_47"), color)); // nil
        data.add(row);

        row = new Vector<Object>();
        row.add(new ColorCell("2-" + Translation.getInstance().getLabel("fs_62"), color));
        if (inode.getIndirect_2() != null)
            row.add(new ColorCell(Integer.toString(inode.getIndirect_2().getId()), color));
        else row.add(new ColorCell(Translation.getInstance().getLabel("fs_47"), color)); // nil
        data.add(row);

        row = new Vector<Object>();
        row.add(new ColorCell("3-" + Translation.getInstance().getLabel("fs_62"), color));
        if (inode.getIndirect_3() != null)
            row.add(new ColorCell(Integer.toString(inode.getIndirect_3().getId()), color));
        else row.add(new ColorCell(Translation.getInstance().getLabel("fs_47"), color)); // nil
        data.add(row);
    ...
    private Vector<Vector<Object>> getIndirectInfoData(IndirectBlock indirect, Color color) {
        ...
        for (int i = 0; i < indirections.size(); i++) {
            ...
            row.add(new ColorCell(Integer.toString(i), color));
            if (block != null) {
                row.add(new ColorCell(Integer.toString(block.getId()), color));
                if (block.isIndirect())
                    row.add(new ColorCell(Translation.getInstance().getLabel("fs_19"), color)); // Indirect
    ...
    public Vector<Vector<Object>> getTableInfoData(Block[] device) {
        ...
        for (int i = getFirstDataBlock(); i < device.length; i++) {
            ...
            if (device[i] != null) {
                ...
                if (!device[i].isIndirect() && !device[i].isInode())
                    row.add(new ColorCell(Translation.getInstance().getLabel("fs_61"), color));
            } else {
    ...
    private int getBlocksNeeded(int objectSize) {
        ...
        if (dataBlocks > limit1Indirect && dataBlocks <= limit2Indirect)
            return dataBlocks + 3 + (((dataBlocks - limit1Indirect - 1) / IndirectBlock.ADDRESS_POINTERS));
        if (dataBlocks > limit2Indirect && dataBlocks <= limit3Indirect)
            return dataBlocks + 5 + IndirectBlock.ADDRESS_POINTERS + (((dataBlocks - limit2Indirect - 1) / IndirectBlock.ADDRESS_POINTERS)) + (((dataBlocks - limit2Indirect - 1) / (IndirectBlock.ADDRESS_POINTERS * IndirectBlock.ADDRESS_POINTERS)));
        return -1; // otherwise
    }
