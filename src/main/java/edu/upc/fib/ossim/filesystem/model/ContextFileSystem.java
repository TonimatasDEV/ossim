...
public class ContextFileSystem {
    ...
    public Vector<Vector<Object>> getSelectedFolderData() {
        ...
        while (it.hasNext()) {
            ...
            row.add(new ColorCell(Integer.toString(item.getFSObject().getId()), item.getColor()));
            data.add(row);
        }
    ...
    public int addFile(int id, Vector<Object> data) throws SoSimException {
        ...
        FileItem file;
        if (id < 0)
            file = new FileItem(fsObject, data.get(0).toString(), folder, (Integer) data.get(1), (Color) data.get(2));
        else
            file = new FileItem(id, fsObject, data.get(0).toString(), folder, (Integer) data.get(1), (Color) data.get(2));
